...
module.exports = function (hoodie) {
	...
	function router(request, reply) {
		...
		//reply('hello router');
		router[request.query.subtype](request, reply);
	};

	router['customers.create'] = function(request, reply) {
			//reply('hello customers.create');
	...
	router['customers.retrieveSubscription'] = function(req, reply) {
		stripe.customersRetrieveSubscription(hoodie, req.query, reply, function(err, message) {
			if (err && err == 'ignore') {
				reply(false);
			} else {
				var options = {};
	...
	router['cleanCouch'] = function(request, reply) {
		stripe.cleanCouch(hoodie, request.query, reply, function(err) {
			if (err && err == 'ignore') {
				reply(false);
			} else {
				reply(true);
