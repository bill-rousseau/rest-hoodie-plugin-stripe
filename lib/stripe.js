...
module.exports.customersCreate = function (hoodie, doc, reply, callback) {
	...
	reply('Creating Stripe Customer...');
...
module.exports.customersUpdateSubscription = function(hoodie, doc, reply, callback) {
	var stripeKey = hoodie.config.get('stripeKey');
	...
	reply('Updating Stripe Subscription...');
...
module.exports.customersRetrieveSubscription = function(hoodie, doc, reply, callback) {
	var stripeKey = hoodie.config.get('stripeKey');
	...
	var handleUserFind = function(error, userDoc) {
		...
		reply('Stripe Subscription Retrieve...');
		hoodie.database(doc.originDb).find('stripe/subscription', doc.planType, function(error, sub) {
			handleSubscriptionFind(error, props, sub);
		});
	};
	reply('Retrieving Stripe Subscription...');
	hoodie.account.find('user', doc.id, function(error, userDoc) {
		handleUserFind(error, userDoc);
	});
};

module.exports.customersCancelSubscription = function(hoodie, doc, reply, callback) {
	var stripeKey = hoodie.config.get('stripeKey');
	...
	reply('Cancelling Stripe Subscription...');
	hoodie.account.find('user', doc.id, function(error, userDoc) {
		handleUserFind(error, userDoc);
	})
};
...
// NOT OK UNLESS YOU CAN RETRIEVE THE FINAL SUBS CLIENT-SIDE
module.exports.customersListSubscriptions = function(hoodie, doc, reply, callback) {
	var stripeKey = hoodie.config.get('stripeKey');
	...
	reply('Listing Stripe Subscriptions...');
	hoodie.account.find('user', doc.id, function(error, userDoc) {
		handleUserFind(error, userDoc);
	})
};

module.exports.chargesCreate = function(hoodie, doc, reply, callback) {
	var stripeKey = hoodie.config.get('stripeKey');
...
module.exports.chargesRetrieve = function(hoodie, doc, reply, callback) {
	var stripeKey = hoodie.config.get('stripeKey');
	...
	reply('Retrieving Stripe Charge...');
	hoodie.account.find('user', doc.id, function(error, userDoc) {
		handleUserFind(error, userDoc);
	})
};
